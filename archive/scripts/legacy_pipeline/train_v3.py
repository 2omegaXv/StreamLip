"""
StreamLip V3 — Phase 1 training (text path only, no FM).

V3 replaces V2's PoE (logit addition) with Flamingo-style cross-attention:
GatedCrossAttentionLayer injected every cross_attn_every_n LM layers.
LM base weights are frozen; only Conformer + cross-attn layers are trained.

Usage:
  python scripts/train_v3.py
  python scripts/train_v3.py --debug
  python scripts/train_v3.py --run_name v3_exp1 --lr 3e-4
"""
import argparse
import math
import os
...
def parse_args():
    ...
    p.add_argument("--output_dir",          default="runs/v3/phase1")
    p.add_argument("--run_name",            default="streamlip_v3_phase1")
    ...
    p.add_argument("--cross_attn_every_n",  type=int,   default=4,
                   help="insert GatedCrossAttn every N LM layers (32/4=8 layers)")
...
@torch.no_grad()
def evaluate(model: StreamLipV3, loader: DataLoader, device: str,
    ...
    for _ in range(100_000):
        for batch in train_loader:
            ...
            if step % args.eval_every == 0:
                val_loss_text, val_acc_word, val_loss_sil, val_acc_sil = evaluate(

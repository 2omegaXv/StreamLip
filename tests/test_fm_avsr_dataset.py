...
class FMAVSRDatasetTest(unittest.TestCase):
    ...
    def test_dataset_can_build_audio_prompt_from_self_non_prefix_window(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            np.savez(
                root / "latent_norm_stats.npz",
            ...
            clip.mkdir(parents=True)
            (clip / "avsr_text.txt").write_text("PROMPT\n")
            np.save(clip / "avsr_enc.npy", np.ones((100, 768), dtype=np.float32))
            latent = np.arange(50 * 512, dtype=np.float32).reshape(50, 512)
            np.savez(clip / "latent.npz", latent=latent)
            np.save(clip / "speaker_emb.npy", np.ones((256,), dtype=np.float32))
            np.save(clip / "smollm2_h.npy", np.ones((1, 960), dtype=np.float16))
            clip_list = root / "clips.txt"
            ...
            ds = FMAVSRDataset(
                ...
                audio_prompt_ref_mode="self_random_window",
            )
            item = ds[0]
    ...
    def test_dataset_can_build_audio_prompt_from_reversed_self_prefix(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            np.savez(
                root / "latent_norm_stats.npz",
            ...
            latent = np.arange(8 * 512, dtype=np.float32).reshape(8, 512)
            np.savez(clip / "latent.npz", latent=latent)
            np.save(clip / "speaker_emb.npy", np.ones((256,), dtype=np.float32))
            np.save(clip / "smollm2_h.npy", np.ones((1, 960), dtype=np.float16))
            clip_list = root / "clips.txt"
            ...
            ds = FMAVSRDataset(
                ...
                clip_list=str(clip_list),
                audio_prompt_frames=4,
                audio_prompt_ref_mode="self_prefix_reverse",
            )
            item = ds[0]
    ...
    def test_parse_args_loads_audio_prompt_cross_attn_pool_tokens_from_config(self):
        old_argv = sys.argv
        try:
            with tempfile.NamedTemporaryFile("w", suffix=".yaml") as f:
                f.write("audio_prompt_cross_attn_pool_tokens: 4\n")
                f.flush()
                sys.argv = [
                    "scripts/train_fm_avsr.py",
                    "--config",

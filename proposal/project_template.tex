...
\midrule
1--2 & Data preparation \& environment setup & LRS3 preprocessed; codec trained/loaded \\
3--4 & Phase 1: AR backbone + text head training & Streaming lip-reading model (WER metric) \\
5--6 & Phase 2: Attach FM head, joint training & End-to-end streaming L2S (audio output) \\
7--8 & Ablation experiments \& hyperparameter tuning & Full evaluation table \\
9    & Analysis, writing, poster preparation & Final report + poster \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Member A (Leader) & Overall architecture design; AR backbone \& text head implementation; loss balancing \\
Member B & Visual encoder integration (AV-HuBERT); data pipeline; look-ahead windowing \\
Member C & FM head implementation; audio codec integration; evaluation pipeline \& ablations \\
\midrule
